...
const AllExercise = ({ navigation, route }) => {
  ...
  const getExercises = async () => {
    try {
      const response = await fetch(
        `https://exercisedb.p.rapidapi.com/exercises/bodyPart/${title.toLowerCase()}?limit=200&offset=0`,
        {
          method: 'GET',
          headers: {
      ...
      const uniqueEquipment = [
        ...new Set(exerciseData.map((ex) => ex.equipment)),
      ].sort();
      const uniqueTargets = [
        ...new Set(exerciseData.map((ex) => ex.target)),
      ].sort();
  ...
  useEffect(() => {
    ...
    setFiltersApplied(
      selectedEquipment.length > 0 || selectedTargets.length > 0
    );
    setCurrentPage(1);
  ...
  return (
    <SafeAreaView style={tw`flex-1 bg-white`}>
      ...
      <ScrollView ref={scrollViewRef} showsVerticalScrollIndicator={false} bounces={true}>
        {/* Hero */}
        ...
        <View style={tw`px-4 mb-4`}>
          <View style={tw`flex-row items-center bg-gray-100 px-4 py-3 rounded-xl`}>
            ...
            <View style={tw`h-6 mx-2 border-r border-gray-300`} />
            <TouchableOpacity onPress={() => setFilterModalVisible(true)} style={tw`p-1`}>
          ...
          {filtersApplied && (
            <View style={tw`flex-row flex-wrap mt-2 items-center`}>
              ...
              <TouchableOpacity onPress={clearFilters} style={tw`bg-gray-200 rounded-full px-3 py-1 mb-2 flex-row items-center`}>
        ...
        <View style={tw`px-4 pb-8`}>
          <View style={tw`flex-row justify-between items-center mb-4`}>
            <Text style={tw`text-lg font-bold text-gray-800`}>Available Exercises</Text>
          ...
          {loading ? (
            ...
            <View style={tw`items-center justify-center py-16 bg-gray-50 rounded-xl`}>
              ...
              {filtersApplied && (
                <TouchableOpacity onPress={clearFilters} style={tw`mt-4 bg-pink-600 rounded-lg px-4 py-2`}>
              ...
              <View style={tw`flex-row flex-wrap justify-between`}>
                {currentExercises.map((item, index) => (
                  ...
                  >
                    ...
                    />
                    <View style={tw`p-2`}>
                      <Text style={tw`text-gray-800 font-semibold text-center`} numberOfLines={1}>
              ...
              <View style={tw`flex-row justify-center items-center mt-6 mb-2 bg-gray-50 py-3 rounded-xl`}>
                ...
                <View style={tw`flex-row mx-2`}>
                  {getPageRange().map((number) => (
                    ...
                    >
                      <Text style={tw`${currentPage === number ? 'text-white font-bold' : 'text-gray-700'}`}>
                        {number}
                      </Text>
                    </TouchableOpacity>
                  ))}
